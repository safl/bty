...
|---|---|---|
| **USB live stick** | bty boots from a flash drive, runs `bty`, flashes the box it's plugged into. The wizard offers a default catalog (Debian / Ubuntu / Fedora / FreeBSD headless images plus a Fedora desktop, via `oras://ghcr.io/safl/nosi/...`) as `[d] default` in SELECT_CATALOG -- no per-stick baking. | Single-machine local imaging |
