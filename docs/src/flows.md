...
| Refuse flash on serial mismatch at boot time           | Live env can't find a current disk whose serial matches cmdline.   | `bty-flash-on-boot` (live env)          | systemd service fails; tty1 message lists current disks + serials. |
