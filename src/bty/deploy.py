...
def init_main(argv: list[str] | None = None, *, prog: str = "bty-lab init") -> None:
    ...
    print(
        f"\nNext: cd {dest} && cp .env.example .env"
        f' && "${{EDITOR:-vi}}" .env && podman compose up -d',
        file=sys.stderr,
    )
    print(
        "      BIOS PXE clients also need TFTP -- "
        "add --profile tftp to the compose command. UEFI HTTP-Boot",
        file=sys.stderr,
    )
    print(
        "      does not. Prereq: podman compose needs a compose backend "
