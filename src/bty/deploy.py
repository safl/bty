...
services:
  ...
  bty-web:
    ...
    environment:
      ...
      # sidecar uses ``network_mode: host``, so udp/69 sits on the host's
      # LAN address -- NOT on bty-web's bridge-network loopback. Default
      # to HOST_ADDR so the in-UI probe actually reaches the sidecar
      # instead of probing bty-web's own (empty) :69. Operators with TFTP
      # on a different machine override BTY_TFTP_PROBE_HOST in envvars.
...
def _quadlet_bty_web(version: str, data_dir_abs: Path, host_addr: str | None = None) -> str:
    ...
    edit_note = (
        ...
        if host_addr is not None
            ...
            "# Edit Environment= lines: replace HOST_ADDR_HERE with this\n"
            "# host's LAN address before installing.\n#\n"
        )
    )
    addr = host_addr or "HOST_ADDR_HERE"
    return f"""\
...
DNS=1.1.1.1
Environment=BTY_WITHCACHE_URL=http://{addr}:3000
# TFTP probe target for the Netboot page diagnostic. bty-tftp uses
# host networking, so udp/69 sits on the host's LAN address (not on
# bty-web's loopback). Same value as BTY_WITHCACHE_URL above.
Environment=BTY_TFTP_PROBE_HOST={addr}
...
def _emit_deploy_files(
    ...
    *,
    ...
    if with_systemd:
        ...
        for fname, body in (
            ("bty-web.container", _quadlet_bty_web(version, data_dir_abs, host_addr)),
