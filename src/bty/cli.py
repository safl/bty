...
def main(argv: list[str] | None = None) -> int:
    ...
    p_login = sub.add_parser(
        ...
        description=(
            ...
            "subsequent CLI invocations can reuse it without re-prompting."
        ),
    )
    ...
    p_logout = sub.add_parser(
        ...
        description=(
            ...
            "(or BTY_TOKEN), then delete the local token file."
        ),
    )
...
DEFAULT_TOKEN_FILE = Path("~/.config/bty/token").expanduser()
...
def cmd_login(args: argparse.Namespace) -> int:
    ...
    if args.password_stdin:
        ...
        password = getpass.getpass(
            f"bty-web password ({args.server}): ",
        )
    if not password:
        print("bty: no password provided; aborting", file=sys.stderr)
        return 1

    body = _json.dumps({"password": password, "label": args.label}).encode("utf-8")
    ...
    try:
        ...
        if exc.code == 401:
            print("bty: invalid credentials", file=sys.stderr)
            return 1
        print(f"bty: login failed: HTTP {exc.code} {exc.reason}", file=sys.stderr)
        return 1
    except urllib.error.URLError as exc:
        print(f"bty: cannot reach {url}: {exc.reason}", file=sys.stderr)
        return 1
    ...
    if args.token_out is not None and str(args.token_out) == "-":
        print(token)
        return 0
    out_path = args.token_out or _default_token_path()
    out_path.parent.mkdir(parents=True, exist_ok=True, mode=0o700)
    out_path.write_text(token + "\n")
    out_path.chmod(0o600)
    print(f"bty: saved session token to {out_path} (expires {payload['expires_at']})")
    return 0
...
def cmd_logout(args: argparse.Namespace) -> int:
    ...
    token_file = args.token_file or _default_token_path()
    if not token_file.is_file():
        print(f"bty: no token file at {token_file}; nothing to do")
        return 0
    token = token_file.read_text().strip()
    if not token:
        token_file.unlink(missing_ok=True)
        print(f"bty: empty token file {token_file} removed")
        return 0
    ...
    try:
        ...
        if exc.code != 401:
            print(f"bty: logout failed: HTTP {exc.code} {exc.reason}", file=sys.stderr)
            return 1
    except urllib.error.URLError as exc:
        print(f"bty: cannot reach {url}: {exc.reason}", file=sys.stderr)
        return 1
    token_file.unlink(missing_ok=True)
    print(f"bty: removed {token_file}")
    return 0
