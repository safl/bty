...
.iso -> initrd -> squashfs) on top of the bty live env's normal
...
Upstream follow-up: cijoe's ``qemu.guest_start`` should accept a
``--timeout`` parameter so projects don't need this fork. Tracked
in [[project_cijoe_upstream_followup]].
