...
def test_upgrade_errors_when_bty_toml_missing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    _patched_runtime: dict[str, list],
) -> None:
    """A deploy directory without bty.toml is no longer an upgrade
    target. ``bty-lab upgrade`` errors with a clear pointer at
    ``bty-lab deploy --force`` instead of silently regenerating
    the config (which would lose the admin password unless it had
    been preserved across the older shape)."""
    dest = tmp_path / "bty-host"
    deploy_mod.deploy_main([str(dest)])
