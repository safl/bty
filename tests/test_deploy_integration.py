...
def _bty_login_cookie() -> str:
    ...
    ``HTTPRedirectHandler`` follows the 303 to ``/ui/`` and reports the
    second response's headers, which have no cookie -- callers then send
    an empty ``Cookie:`` header and every subsequent request 401s.
    Intercept the 303 so we can read Set-Cookie off it.
    """
    import urllib.error
    import urllib.request
